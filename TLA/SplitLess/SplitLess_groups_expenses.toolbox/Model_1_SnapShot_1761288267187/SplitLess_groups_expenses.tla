...
ComputeGifts(grp, balances) ==
  LET giftingUsers == { u \in USERS : grp.members[u] = FALSE /\ balances[u] > 0 } \* all users not in group with positive balance (they gift sth)
...
CreateGroup ==
    ...
    \E gid \in POSSIBLE_GROUP_IDs :
        /\ groups[gid] = NO_GROUP
        /\ LET newGroup == [ id |-> gid, creator |-> actor, members |-> [u \in USERS |-> IF u = actor THEN TRUE ELSE FALSE], expenseids |-> {}, totalGifted |-> 0, individualGiftsSent |-> [u \in USERS |-> 0], passiveMembers |-> [u \in USERS |-> FALSE]]
...
AddExpenseToGroup ==
        ...
        \E gid \in POSSIBLE_GROUP_IDs :
            ...
            /\ groups[gid].members[recordedExpenses[eid].payer] = TRUE \* payer could have shares 0
...
\* Last modified Thu Oct 23 16:16:32 CEST 2025 by floyd
